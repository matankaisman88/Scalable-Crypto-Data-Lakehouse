...
Steps:
  0. Fetch  — download yesterday's raw kline ZIPs from Binance public S3
              directly via boto3 (no Docker daemon / AWS credentials needed).
  1. Bronze — CSV/ZIP → Delta
  2. Silver — dedup, cast, metadata join
  3. Gold   — OHLCV aggregation
...
def run_refresh(target_date: Optional[str] = None) -> Iterator[str]:
    ...
    yield ""
    yield "─" * 50
    yield f"[0/3] Fetching raw data from Binance S3 for {ingest_date} …"
    yield "─" * 50
    try:
        ...
        yield f"[0/3] WARNING: Fetch failed ({exc}). Continuing with existing raw data …"
    ...
    yield ""
    yield "─" * 50
    yield f"[1/3] Bronze ingestion for {ingest_date} …"
    yield "─" * 50
    try:
    ...
    yield ""
    yield "─" * 50
    yield f"[2/3] Silver transformation for {ingest_date} …"
    yield "─" * 50
    try:
    ...
    yield ""
    yield "─" * 50
    yield f"[3/3] Gold aggregations for {ingest_date} …"
    yield "─" * 50
    try:
